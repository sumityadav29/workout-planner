...
- The first unchecked slot is the "current activity": `render()` gives that row (or the run block) the day-accent tint, checkbox ring, and a Next chip via `nextIdx`, and tags it `id="nextrow"`. Checking a slot smooth-scrolls to the new next (`scrollToNext()`); a finished rest timer auto-closes after 1.5s and pulses it (`pulseNext()`, `.pulse-next` keyframes).
