...
- Only today's plan is editable by default (`isEditable()`): other days render read-only — disabled checkboxes, no run/reset buttons — with a small padlock chip beside the day-type badge that unlocks that day via `unlockDay()` (session-only `unlocked` set, resets on reload).
