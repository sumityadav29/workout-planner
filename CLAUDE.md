...
`PLAN` is **per-user**: the hardcoded array is only `DEFAULT_PLAN` (the signed-out/first-run fallback — edit it, not the markup, for default-plan changes). Signed in, `pullPlan()` fetches the user's row from the Supabase `plans` table (one `jsonb` row per user, same shape as `DEFAULT_PLAN`; seeded automatically on first sign-in), caches it in localStorage `wp-plan`, and `initPlan()` rebuilds `done`/re-renders when it differs. The only in-app plan editing is drag reordering: exercises can be reordered within their section via tap-hold drag (SortableJS CDN; `initSortables()` on editable days, `delayOnTouchOnly` so touch needs a ~300ms hold). `reorderEx()` splices `PLAN` and the day's `done` slots together (checkmarks travel with their exercise, run slot 0 fixed), then `savePlan()`/`pushPlan()` cache and upsert the plan with a `wp-plan-dirty` flag for offline retry — a dirty local plan wins over the server copy in `pullPlan()`. Other plan edits happen as `plan` jsonb in the Supabase dashboard. Past calendar days are judged by their own stored slot arrays (`dayState`), so plan changes never rewrite history.
