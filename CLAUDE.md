...
`PLAN` is **per-user**: the hardcoded array is only `DEFAULT_PLAN` (the signed-out demo — edit it, not the markup, for default-plan changes). Signed in, `pullPlan()` fetches the user's row from the Supabase `plans` table (one `jsonb` row per user, same shape as `DEFAULT_PLAN`), caches it in localStorage `wp-plan`, and `initPlan()` rebuilds `done`/re-renders when it differs. A new account has no row: `pullPlan()` opens the `#pdlg` chooser (`openPlanChoice()`, no backdrop dismiss) and `choosePlan()` seeds either a copy of `DEFAULT_PLAN` or `blankPlan()` — seven empty `type: 'Open'` days that render a "Nothing planned yet" state and get built via "+ Add exercise". `signOut()` clears `wp-plan`/`wp-plan-dirty` and restores `DEFAULT_PLAN` so the next account on the device never inherits or pushes the previous user's plan. In-app plan editing (editable days only) is: drag reordering — exercises reorder within their section via tap-hold drag (SortableJS CDN; `initSortables()`, `delayOnTouchOnly` so touch needs a ~300ms hold), with `reorderEx()` splicing `PLAN` and the day's `done` slots together (checkmarks travel with their exercise, run slot 0 fixed); an "+ Add exercise" button under the checklist (`openAdd()`/`addExercise()`, `#adlg` dialog — name required, muscles/sets default to `Custom`/`3 × 10`, rest defaults to 60s, rest 0 makes a no-rest exercise; appends to the day's last section, creating one on run-only days); and a per-row × button (`removeEx()`) that splices the exercise and its `done` slot out. Every plan edit runs `persist()` + `savePlan()`/`pushPlan()`, which cache and upsert the plan with a `wp-plan-dirty` flag for offline retry — a dirty local plan wins over the server copy in `pullPlan()`. Bigger edits happen as `plan` jsonb in the Supabase dashboard. Past calendar days are judged by their own stored slot arrays (`dayState`), so plan changes never rewrite history.

Because plan text is user-defined, **anything from `PLAN` interpolated into an HTML template string must go through `esc()`** (the HTML-entity escaper defined next to `PLAN`), and inline `onclick` handlers must pass indices, not strings (e.g. `openTimerAt(i)` resolves the slot index to its exercise via `exAt()` instead of embedding the exercise name).
...
- Only today's plan is editable by default (`isEditable()`): other days render read-only — disabled checkboxes, no run/reset buttons — with a padlock chip beside the day-type badge that toggles editability via `toggleLock()` — closed lock on white when locked, open lock on yellow when unlocked (session-only `unlocked` set, resets on reload).
